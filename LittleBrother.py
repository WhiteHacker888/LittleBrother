...
mainOption = """
 [1] Izlash
 [2] Boshqa tool(asbob)lar
 [3] Profiler
 [4] Mamlakatni uzgartirish

 [e] Chiqish    [h] Yordam xabari    [c] Ekranni tozalash"""


lookupOption = """
 [1] Odam izlash          [8] Mail joylashuv aniqlash
 [2] Username izlash          [9] Employés recherche
 [3] Manzil izlash           [10] Google qidiruv
 [4] Raqam izlash             [11] Facebook GraphSearch
 [5] IP izlash                [12] twitter info
 [6] SSID locator             [13] instagram info
 [7] Email izlash

 [b] bosh menyuga qaytish    [e] Chiqish    [h] yordam xabari    [c] ekranni tozalash"""
...
profilerOption = """
 [1] Profiler
 [2] Barcha profile larni kursatish
 [3] Profile yaratish

 [b] bosh menyuga qaytish    [c] ekranni tozalash   [h] yordam xabari
"""
			...
			while True:
				...
				if choix.lower() == 'h':
					...
					if pr.count >= 1:
						...
						print(warning+" Hech qanday profil topilmadi.  Iltimos, birini yarating..")
				elif choix.lower() == "2":
					pr.showAllProfiles(database=database)

				elif choix.lower() == '3':
					print("\n"+Fore.YELLOW+"(Format: Ism familiyasi)"+Fore.RESET)
					while True: 
						name = input(" Profile nomi: ")
						if name != '':
							break
					name = name.split(" ")
					...
					name = " ".join(name)
					while True:
						print(question+" Profil uchun Twitter hisobini royxatdan otkazmoqchimisiz ?")
						choixPr = input(" [O/n]: " )
						if choixPr.upper() == 'N':
							break
						else:
					...
					while True:
						print(question+" Profil uchun Instagram hisob qaydnomasini royxatdan otkazmoqchimisiz ?")
						choixPr = input(" [O/n]: " )
						if choixPr.upper() == 'N':
							break
						else:
							...
							break
					while True:
						print(question+" Profil uchun Facebook hisob qaydnomasini royxatdan otkazmoqchimisiz ?")
						choixPr = input(" [O/n]: " )
						if choixPr.upper() == 'N':
							break
						else:
					...
					if create:
						print("\n"+found+" Profil '%s' muvaffaqiyatli yaratildi." % (name))
					else:
						print("\n"+warning+" Xatolik yuz berdi.  Profil '%s' yaratish mumkin emas edi." % (name))
