...
def profilerFunc(profile='', path=''):
	...
	%s
			...
			print(question+" Qayta tiklangan malumotlarni eksport qilishni xohlaysizmi '%s' ? " % (name_txt))
			...
			while True:
				...
				if choix == '' or choix.upper() == 'O':
					...
					if f:
						print("\n"+found+" Malumot muvaffaqiyatli eksport qilindi !")
						print(" %s" % (path_txt))
					else:
						print("\n"+warning+" Xatolik yuz berdi, malumotlarni eksport qilib bolmadi !")
					break

				elif choix.upper() == 'N':
					break

			print("\n"+question+" Nusxasini yaratmoqchimisiz? '%s' ? " % (name_txt))
			
			while True:
				choix = input("\n [o/N]: ")

				if choix.upper() == 'O':
					print("\n"+question+" Yoki nusxasini saqlamoqchimisiz ?")
					pathDefault = os.getcwd()
					print(Fore.YELLOW+" Standart manzil: "+pathDefault+Fore.RESET)
					path = input("\n Manzil: ")
					...
					print("\n"+found+" '%s' muvaffaqiyatli nusxa kochirildi !" % (name_txt))
					break
